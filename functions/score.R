...
eu = function(common.peaks, tolerance = common.peaks$tolerance) {
  peaks1.common = common.peaks[[1]]
  peaks2.common = common.peaks[[2]]
  peaks1.n = nrow(common.peaks[[3]])
  peaks2.n = nrow(common.peaks[[4]])
  peaks1.common.n = nrow(peaks1.common)
  if(peaks1.common.n == 0)
    return(0)
  
  delta.e = ((peaks1.common[, 1] - peaks2.common[, 1]) ^ 2 + (peaks1.common[, 2] - peaks2.common[, 2]) ^ 2) ^ 0.5
  i.max = apply(cbind(peaks1.common[, 2], peaks2.common[, 2]), 1, max)
  mz.max = apply(cbind(peaks1.common[, 1], peaks2.common[, 1]), 1, max)
  relative.distance = delta.e / (i.max ^ 2 + (mz.max * tolerance * 1e-6) ^ 2) ^ 0.5
  eu.score = abs((sum(relative.distance) + peaks1.n + peaks2.n - (2 * peaks1.common.n)) /
                   (peaks1.n + peaks2.n -  peaks1.common.n))
  return(1 - eu.score)
}

ieu = function(common.peaks, tolerance = common.peaks$tolerance) {
  peaks1.common = common.peaks[[1]]
  peaks2.common = common.peaks[[2]]
  ...
  mz.max = apply(cbind(peaks1.common[, 1], peaks2.common[, 1]), 1, max)
  relative.distance = delta.e / (i.max ^ 2 + (mz.max * tolerance * 1e-6) ^ 2) ^ 0.5
  ieu.score = (sum(relative.distance * (peaks1.common[, 2] + peaks2.common[, 2])) + 
                 (sum(peaks1[, 2]) + sum(peaks2[, 2]) - sum(peaks1.common[, 2]) - sum(peaks2.common[, 2]))) /
    (sum(peaks1[, 2]) + sum(peaks2[, 2]))
  return(1 - ieu.score)
}
...
score = function(peaks1, peaks2, tolerance, method = c('eu', 'ieu', 'cosine')) {
